							 ...
							 e_mt_gen_alg mt_alg, e_sharing sharing, double afp, double bfp)
{
	...
	std::string circuit_dir = "../extern/ABY/bin/circ/";
	...
	float bfloat = (float)bfp;
	uint32_t *bfloatptr = (uint32_t *)&bfloat;
	...
	share *bsqrtin = circ->PutINGate(bfloatptr, 32, SERVER);
	...
	share *div_share = circ->PutFPGate(asqrtin, bsqrtin, DIV);
	...
	share *div_out = circ->PutOUTGate(div_share, ALL);
	...
	uint32_t *div_out_vals = (uint32_t *)div_out->get_clear_value_ptr();

	float val = *((float *)sqrt_out_vals);
	float div_val = *((float *)div_out_vals);

	std::cout << "SQRT RES: " << val << " = " << sqrt(afloat) << std::endl;
	std::cout << "DIV RES: " << div_val << " = " << afloat / bfloat << std::endl;
}
